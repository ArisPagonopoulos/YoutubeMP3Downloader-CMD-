...
def downloader(name,method="slow"):
    ...
    if  not os.path.isdir(f"c:\\users\\{user}\\desktop\\mp3downloads"):
        os.mkdir(f"c:\\users\\{user}\\desktop\\mp3downloads")
    os.chdir(f"c:\\users\\{user}\\desktop\\mp3downloads")
    ...
    if name.strip():
        ...
        if method=="slow":
            ...
            choice=int(input("Choose the song you want to download: type 1 and enter if you want the first, 2 and enter for the second etc.\nType 0 for more options: "))
            if choice==0:
                os.system("cls")
